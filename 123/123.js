...
GET_CARDS_RESPONSE.forEach(function(card) {
    ...
    let optionList = GET_USERS_RESPONSE.map(user => {
        let isCurrentUser = user.id === card.userId;

        return `<option ${isCurrentUser ? "selected": ''} value="${user.id}">${user.name}</option>`;
...
// let srav = GET_COLUMNS_RESPONSE.name === GET_COLUMNS_RESPONSE.